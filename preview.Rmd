...
-   Las cajas que son el rango intercuartílico (Del primer al tercer cuartil), que muestra la dispersión central de los datos. Ambos géneros están igual distribuidos.
...
## Influencia de motivación en las variables de correlación mas elevadas.
...
### Análisis de nota de examen por nivel de motivación.

HAY QUE CAMBIARLO TODO PORQUE EN RESUMEN ES QUE NO INFLUYE NADA EL NIVEL DE MOTIVACIÓN EN LA NOTA, NADA O MINIMAMENTE
...
Vemos que las motivaciones bajas tienen notas mas bajas, las notas medias se concentran en notas mayores que las motivaciones altas, sin embargo las motivaciones altas tienen las mejores notas

### Análisis de asistencia por nivel de motivación
...
ggplot(st_clean, aes(x = Motivation_Level, y = Attendance, fill = Motivation_Level)) +
  geom_boxplot() +
  theme_minimal() +
  labs(title = "Diagrama de Caja de asistencia por Nivel de Motivación",
...
ggplot(st_clean, aes(x = Hours_Studied, y = Exam_Score, color = Motivation_Level)) +
  geom_point(alpha = 0.7) +
  theme_minimal() +
  labs(title = "Diagrama de Dispersión de horas de estudio vs nota de examen",
       x = "Horas de estudio",
       y = "notas del Examen",
       color = "Nivel de Motivación") +
  scale_color_brewer(palette = "Set1")
...
ggplot(st_clean, aes(x = Hours_Studied, fill = Motivation_Level)) +
  geom_density(alpha = 0.5) +
  theme_minimal() +
  labs(title = "Gráfico de horas de estudio por Nivel de Motivación",
       x = "horas estudiadas",
       y = "Densidad",
       fill = "Nivel de Motivación") +
  scale_fill_manual(values = c("High" = "lightblue", "Medium" = "lightgreen", "Low" = "lightpink"))
...
### Análisis de calidad de profesor
...
Vamos a analizar cuanto influyen las horas de tutoria en el resultado final.

```{r}
ggplot(st_clean, aes(x = as.factor(Tutoring_Sessions), y = Exam_Score, fill = as.factor(Tutoring_Sessions))) +
  geom_boxplot() +
  theme_minimal() +
  labs(title = "Diagrama de Caja de nota de examen por Número de Sesiones de Tutoría",
...
DE ESTE PUEDO SACAR ALGO DE LA CATEGÓRICA
...
Según nuestra correlación podríamos enfocarnos en predecir la nota del examen con un modelo de regresión lineal o múltiple de las variables numéricas que tienen el dato de correlación positiva mas alta que son: `Hours_Studied` y `Attendance`. Podríamos mirar luego si usar un modelo basado en árbol como random forest o gradient boosting.
