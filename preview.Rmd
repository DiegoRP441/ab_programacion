...
![![](images/clipboard-1662967905.png)](images/clipboard-212070460.png)
...
Primero cargamos las librerías que usaremos en R
...
Luego cargamos las librerías que usaremos en python
...
Importamos el dataset y mostramos las 10 primeras filas de nuestro dataset
...
Como las 3 columnas que contienen nulos son categóricas vamos a limpiarlas usando la librería `pandas` de python para una mayor facilidad a la hora de trabajar con la moda.
...
primero veremos si existe algún tipo de desequilibrio en los datos
...
-   **Género**: La distribución es equilibrada.
-   **Ingreso Familiar**: Predominan las clases media y baja.
-   **Tipo de Escuela**: Mayor representación de escuelas públicas.
...
grafico_motivacion <- ggplot(motivacion_por_genero, aes(x = Gender, y = Percentage, fill = Motivation_Level)) +
  geom_bar(stat = "identity", position = "dodge") +
  theme_minimal() +
  labs(title = "Distribución porcentual de Motivación",
       x = "Género",
       y = "Porcentaje",
       fill = "Motivación") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
grafico_tutoria <- ggplot(promedio_tutoria_por_genero, aes(x = Gender, y = Promedio_Tutoring_Sessions, fill = Gender)) +
  geom_bar(stat = "identity", position = "dodge") +
  theme_minimal() +
  labs(title = "Promedio de Horas de Tutoría",
       x = "Género",
       y = "Promedio de Horas",
       fill = "Género") +
...
grafico_estudio <- ggplot(promedio_estudio_por_genero, aes(x = Gender, y = Promedio_Study_Hours, fill = Gender)) +
  geom_bar(stat = "identity", position = "dodge") +
  theme_minimal() +
  labs(title = "Promedio de Horas de Estudio",
       x = "Género",
       y = "Promedio de Horas",
       fill = "Género") +
...
grafico_asistencia <- ggplot(promedio_asistencia_por_genero, aes(x = Gender, y = Promedio_Attendance, fill = Gender)) +
  geom_bar(stat = "identity", position = "dodge") +
  theme_minimal() +
  labs(title = "Promedio de Asistencia",
       x = "Género",
...
barplot_exam <- st_clean %>%
  ...
  geom_bar(stat = "identity", position = "dodge") +
  theme_minimal() +
  labs(title = "Media de Exam_Score",
       x = "Learning Disabilities",
       y = "Media de Notas del Examen") +
  theme(legend.position = "none",
        axis.text.x = element_text(angle = 45, hjust = 1))
...
boxplot_previous <- ggplot(st_clean, aes(x = Learning_Disabilities, y = Previous_Scores, fill = Learning_Disabilities)) +
  geom_boxplot() +
  theme_minimal() +
  labs(title = "Distribución de Previous_Scores",
       x = "Learning Disabilities",
       y = "Notas Previas") +
  theme(legend.position = "none",
        axis.text.x = element_text(angle = 45, hjust = 1))
...
barplot_previous <- st_clean %>%
  ...
  geom_bar(stat = "identity", position = "dodge") +
  theme_minimal() +
  labs(title = "Media de Previous_Scores",
       x = "Learning Disabilities",
       y = "Media de Notas Previas") +
  theme(legend.position = "none",
        axis.text.x = element_text(angle = 45, hjust = 1))
...
grafico_peer_influence <- st_clean %>%
  ...
  geom_bar(stat = "identity", position = "dodge") +
  theme_minimal() +
  labs(title = "Porcentaje de Peer Influence",
       x = "Learning Disabilities",
       y = "Porcentaje",
       fill = "Peer Influence") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
grafico_parental_involvement <- st_clean %>%
  ...
  geom_bar(stat = "identity", position = "dodge") +
  theme_minimal() +
  labs(title = "Porcentaje de Parental Involvement",
       x = "Learning Disabilities",
       y = "Porcentaje",
...
grafico_family_income <- st_clean %>%
  ...
  theme_minimal() +
  labs(title = "Distribución de Family Income",
       x = "School Type",
       y = "Porcentaje",
       fill = "Family Income") +
...
grafico_teacher_quality <- st_clean %>%
  ...
  geom_bar(stat = "identity", position = "fill") +
  theme_minimal() +
  labs(title = "Distribución de Teacher Quality",
       x = "School Type",
       y = "Porcentaje",
...
Bien sea público o privado el tipo de escuela no parece tener relevancia las notas.
...
No parece existir ningún tipo de correlación entre las horas de sueño con las notas del examen.

## Conclusión de los análisis de los gráficos

Tras el análisis de varios factores desde múltiples perspectivas, los resultados que se obtuvieron en las todas las comparaciones fueron consistentes. Esto nos sugiere que las variables evaluadas no tienen un impacto significativo en el desempeño de los estudiantes, o bien el dataset ha sido diseñado de manera que solo una pocas variables, específicamente las 3/4 columnas numéricas que luego especificaremos, poseen una influencia real en los resultados.

## Análisis de las 4 correlaciones mayores

Si observamos los resultados del mapa de correlaciones nos da que hay 4 variables numéricas que tienen una relación significativa con la variable a predecir.

variables predictivas serán:
...
y la variable a predecir será: `Exam_Score.`

Analicemos visualmente la distribución de estas 4 variables.
...
Se observa que la distribución de los datos sigue una tendencia lineal en la mayoría de los casos, lo que sugiere una posible relación proporcional entre las variables analizadas. Para evaluar con mayor precisión el grado de ajuste de esta relación, sera necesario incorporar una línea de regresión que permita visualizar cómo se alinean los datos con respecto a una tendencia esperada.
...
Las **métricas del modelo** indican su rendimiento y ajuste. El **R² (0.572)** muestra que el modelo explica el **57.2%** de la variabilidad en **Exam_Score** a partir de las variables **Attendance**, **Hours_Studied** y **Previous_Scores**, mientras que el restante **42.8%** se debe a factores no considerados en el modelo.
...
Vemos que al añadir la variable extra de `Tutoring_Sessions` mejora el r2.
...
El **Modelo 2**, que incluye la variable **`Tutoring_Sessions`**, es mejor que el **Modelo 1** porque tiene un **R² mayor** (59.67% frente a 57.21%), lo que significa que explica mejor la variabilidad de **`Exam_Score`**. Aunque la mejora es moderada, demuestra que **`Tutoring_Sessions`** aporta información útil al modelo.
...
El modelo nos da un R2 un poco superior, 0.61
...
Al ver que el r2 de los modelos probados usando la regresión lineal es muy baja probaremos a usar otro modelo, en este caso usaremos `Naive Bayes`
...
Convertimos la columna `Exam_Score` en una columna categórica.
...
Vamos a realizar el ANOVA de las variables categóricas en relación con `exam_score`. Si el p-valor es inferior a 0,05 la relación es significativa, si es superior la relación no es significativa. Usaremos todas las variables que salgan con una relación significativa en el modelo, ademas de las 4 numéricas que ya hemos comprado antes en el mapa de correlaciones que tienen relación.
...
Tras los resultados del examen de ANOVA concluimos que podemos quedarnos con 11 de 13
...
Ademas eliminamos `Exam_Score` porque la hemos cambiado por `Exam_Grade`
...
Comprobamos si hay algún valor nulo antes de crear el modelo para evitar errores.
...
Nos da una precisión del 0.98, es un dato que indica que hay un **overfitting**, ademas vemos que la clase `distinction` tiene un rendimiento de 0. Esto puede significar un desbalanceo de clases.

Vamos a observar como están las clases de desbalanceadas.
...
Podemos observar que las clases están muy desbalanceadas. Vamos a realizar un sobremuestreo para igualar las clases.
...
# Esto lo usamos para convertir las columnas categoricas a númericas y poder usarlas.
preprocessor = ColumnTransformer(
    transformers=[
...
Tras el sobremuestreo los datos son devueltos como arrays de NumPy y por ello lo convertimos en un dataframe para poder usarlo con mayor facilidad.
...
X = data2.drop(columns=['Exam_Grade'])
y = data2['Exam_Grade']
...
Tras el sobremuestreo realizado nos da una precisión de 0.8 que es un buen dato de precisión.

## Conclusión de los modelos.

Tras realizar varias pruebas con los modelos de regresión lineal y Naive Bayes hemos concluido que el mejor modelo es Naive Bayes ya que nos da una `accuracy` del 0.8 bastante superior a la obtenido con la regresión lineal de un r2 de 0.6.
