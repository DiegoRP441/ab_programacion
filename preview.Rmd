...
![](images/clipboard-3608310708.png)

![](images/clipboard-660723890.png)
...
st['Teacher_Quality'] = st['Teacher_Quality'].fillna(st['Teacher_Quality'].mode()[0])
st['Parental_Education_Level'] = st['Parental_Education_Level'].fillna(st['Parental_Education_Level'].mode()[0])
st['Distance_from_Home'] = st['Distance_from_Home'].fillna(st['Distance_from_Home'].mode()[0])

print("las modas usadas para rellenar los nulos son los siguientes:",st['Teacher_Quality'].mode()[0],",",st['Parental_Education_Level'].mode()[0],"," ,st['Distance_from_Home'].mode()[0])
```
...
```{python}
st.to_csv("st_clean.csv", index=False)
...
Según vemos en el mapa de correlaciones y analizando el resto de variables y las rectas de regresión correspondientes concluimos que vamos a utilizar una **Regresión lineal múltiple** para predecir el `exam_score`. las variables que incluiremos en esta regresión lineal múltiple serán las siguientes: `attendance`, `hours_studied` y `Previous_score` por orden de peso. \### ¿Es necesario aplicar un submuestreo? Como hemos visto al principio del análisis hay algunas variables categóricas que están desbalanceadas, vamos a comprobar si estas tienen relación con las variables continuas que usaremos para el modelo.

### ¿Submuestreo?

Para ver si es necesario hacer un submuestreo debido al desequilibrio en los datos comprobamos el p valor entre las categóricas: `family_income, school_type, learning_disabilities, gender` con las variables predictoras que vamos a usar: `Attendance, Hours_Studied, Previous_Scores, Tutoring_Sessions`.

El **ANOVA** (Análisis de Varianza) y el **t-test** (Prueba t de Student) son herramientas estadísticas utilizadas para comparar las medias de una variable continua entre grupos categóricos. El ANOVA se aplica cuando hay tres o más grupos, mientras que el t-test se utiliza para comparar dos grupos. Ambas pruebas determinan si las diferencias observadas entre las medias de los grupos son estadísticamente significativas. Si los resultados arrojan un **p-valor mayor a 0.05**, se concluye que no hay diferencias significativas entre los grupos, lo que indica que las variables categóricas no influyen en las continuas. En este caso, esto justifica no realizar submuestreo, ya que el desbalance de las categorías no afecta el análisis ni el modelo predictivo.

```{r}
anova_family_income <- aov(Attendance ~ Family_Income, data = st_clean)
summary(anova_family_income)[[1]]["Pr(>F)"]

anova_school_type <- aov(Attendance ~ School_Type, data = st_clean)
summary(anova_school_type)[[1]]["Pr(>F)"]

anova_learning_disabilities <- aov(Attendance ~ Learning_Disabilities, data = st_clean)
summary(anova_learning_disabilities)[[1]]["Pr(>F)"]

t_gender <- t.test(Attendance ~ Gender, data = st_clean)
t_gender$p.value

anova_family_income <- aov(Hours_Studied ~ Family_Income, data = st_clean)
summary(anova_family_income)[[1]]["Pr(>F)"]

anova_school_type <- aov(Hours_Studied ~ School_Type, data = st_clean)
summary(anova_school_type)[[1]]["Pr(>F)"]

anova_learning_disabilities <- aov(Hours_Studied ~ Learning_Disabilities, data = st_clean)
summary(anova_learning_disabilities)[[1]]["Pr(>F)"]

t_gender <- t.test(Hours_Studied ~ Gender, data = st_clean)
t_gender$p.value

anova_family_income <- aov(Previous_Scores ~ Family_Income, data = st_clean)
summary(anova_family_income)[[1]]["Pr(>F)"]

anova_school_type <- aov(Previous_Scores ~ School_Type, data = st_clean)
summary(anova_school_type)[[1]]["Pr(>F)"]

anova_learning_disabilities <- aov(Previous_Scores ~ Learning_Disabilities, data = st_clean)
summary(anova_learning_disabilities)[[1]]["Pr(>F)"]

t_gender <- t.test(Previous_Scores ~ Gender, data = st_clean)
t_gender$p.value

anova_family_income <- aov(Tutoring_Sessions ~ Family_Income, data = st_clean)
summary(anova_family_income)[[1]]["Pr(>F)"]

anova_school_type <- aov(Tutoring_Sessions ~ School_Type, data = st_clean)
summary(anova_school_type)[[1]]["Pr(>F)"]

anova_learning_disabilities <- aov(Tutoring_Sessions ~ Learning_Disabilities, data = st_clean)
summary(anova_learning_disabilities)[[1]]["Pr(>F)"]

t_gender <- t.test(Tutoring_Sessions ~ Gender, data = st_clean)
t_gender$p.value
```

Dado que las pruebas estadísticas respaldan la independencia entre las variables categóricas y las continuas, y considerando que las variables categóricas no aportan una relación significativa al análisis, no es necesario ni justificado realizar submuestreo en este caso. Proceder con el conjunto de datos original asegura la máxima retención de información y la construcción de un modelo más robusto.
...
cat("Porcentaje de aciertos (±2.4 puntos) - Modelo 1:", round(aciertos1, 2), "%\n")
cat("Porcentaje de aciertos (±2.4 puntos) - Modelo 2:", round(aciertos2, 2), "%\n")
...
Hemos elegido un margen de error de **±2.4 puntos** porque se ajusta a la **variabilidad de los residuos** observada en el modelo. En concreto, el **residual standard error** (error estándar residual)
...
predicciones <- predict(modelo_cv, data)

resultados <- data.frame(
  Real = data$Exam_Score,
  Predicho = predicciones
)

margen <- 2.4
resultados$Exito <- abs(resultados$Real - resultados$Predicho) <= margen

porcentaje_exito <- mean(resultados$Exito) * 100
